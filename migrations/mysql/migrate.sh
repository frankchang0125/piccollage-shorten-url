...
docker run -v ${DIR}:/migrations --network host migrate/migrate -path=/migrations/piccollage -database mysql://${MYSQL_ROOT_USER}:${MYSQL_ROOT_PASSWORD}@tcp\(${MYSQL_HOST}:${MYSQL_PORT}\)/piccollage up
